...
class ValueDataset2(Dataset2):
    def __init__(self, examples, fields, dictionary):
        self.dictionary = dictionary
        super(ValueDataset2, self).__init__(examples, fields)

    def collate_fn(self):
        def collate(batch):
            value_batch_flattened = list(itertools.chain(*batch))
            value_tensors_flattened = super(ValueDataset2, self).collate_fn()(value_batch_flattened)
    ...
    def __getitem__(self, i):
        ...
        for idx in self.dictionary[i]:
            value_data = super(ValueDataset2, self).__getitem__(idx)
...
class DictionaryDataset2(Dataset2):
    def __init__(self, examples, fields, key_fields):
        ...
        self.key_dataset = Dataset2(key_examples, key_fields)
        self.value_dataset = ValueDataset2(value_examples, value_fields, dictionary)
        super(DictionaryDataset2, self).__init__(examples, fields)
...
class PairedDataset2(Dataset2):
    def __init__(self, examples, fields):
        assert ('image' in fields)
        assert ('text' in fields)
        super(PairedDataset2, self).__init__(examples, fields)
    ...
    def image_set(self):
        ...
        dataset = Dataset2(examples, {'image': self.image_field})
        return dataset
    ...
    def text_set(self):
        ...
        dataset = Dataset2(examples, {'text': self.text_field})
        return dataset

    def image_dictionary(self, fields=None):
        if not fields:
            fields = self.fields
        dataset = DictionaryDataset2(self.examples, fields, key_fields='image')
        return dataset

    def text_dictionary(self, fields=None):
        if not fields:
            fields = self.fields
        dataset = DictionaryDataset2(self.examples, fields, key_fields='text')
        return dataset
...
class COCO2(PairedDataset2):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        super(COCO2, self).__init__(examples, {'image': image_field, 'text': text_field})

    @property
    def splits(self):
        train_split = PairedDataset2(self.train_examples, self.fields)
        val_split = PairedDataset2(self.val_examples, self.fields)
        test_split = PairedDataset2(self.test_examples, self.fields)
